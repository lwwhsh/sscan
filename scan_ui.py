...
from xafsScanMonitor import ScanDisplay
...
class SscanBaseMainFrame(sscan_gui_autogenerated.MainFrame):
    def __init__(self, parent):
        ...
        self.plotYdata = []
        self.plotXdata = []
        self.plots = ScanDisplay(self)
        # self.plots.Show(True)
        ...
        self.scan2 = Scan('BL10C:scan2')                            # scan record
        ...
        self.scanAbortPV = epics.PV("BL10C:AbortScans.PROC")  # scan Abort PV
    ...
    def StartScan(self, event):
        """
        Generate xafs step scan positions(P1PA) and then START scan
        """
        # Todo: check scanning or not!
        self.e0EdgeEnergy = float(self.m_SetEdgeEnergy.Value)
        self.scanPosGen(self.e0EdgeEnergy, self.reg_settings)

        # Todo: we need DCM pre postion set to -200[eV] before scan start(e0-startE-200ev)
    ...
    def ShowPlot(self, event):
        """
        Open XAFS Scan viewer
        1st check previouse active plot panel
        2nd open if not opened previouse plot panel
        """
        for x in self.GetChildren():
                if isinstance(x, ScanDisplay):
                    return

        self.plots = ScanDisplay(self)
        if not self.scanBusy:
            kwarg = {'plotXdata': self.xEnergy, 'plotYdata': self.yTrans}
            self.plots.onUpdatePlot(**kwarg)
        return
    ...
    def scanBusyCallBack(self, **kwargs):
        if kwargs['value'] == 1:  # Now scanning state
            self.pause_button.Enable()
            self.resume_button.Disable()
